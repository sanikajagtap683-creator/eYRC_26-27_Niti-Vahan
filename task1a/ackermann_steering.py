...
def ackermann_wheel_angles(delta):
    ...
    turning_radius = WHEELBASE / math.tan(delta)

    if delta > 0:
        left_angle = math.atan(WHEELBASE / (turning_radius - half_track))
        right_angle = math.atan(WHEELBASE / (turning_radius + half_track))
    else:
        left_angle = math.atan(WHEELBASE / (turning_radius + half_track))
        right_angle = math.atan(WHEELBASE / (turning_radius - half_track))
